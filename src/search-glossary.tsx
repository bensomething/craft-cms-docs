    ...
    >
      ...
      {!isBusy &&
        (shouldGroup
          ...
          : items.map((t) => <Row key={t.id} term={t} hasMore={hasMore} onLoadMore={handleLoadMore} />))}
    </List>
  );
}
...
function TermDetail({ term }: { term: GlossaryTerm }) {
  ...
  return (
    <List.Item.Detail markdown={buildMarkdown(term)} metadata={hasDocs ? <DocsMetadata term={term} /> : undefined} />
  );
}
...
function DocsMetadata({ term }: { term: GlossaryTerm }) {
  ...
  return (
    <List.Item.Detail.Metadata>
      ...
      {docs.map((d, i) => (
        <List.Item.Detail.Metadata.Link key={`${term.slug}-doc-${i}`} title={d.title} text="View" target={d.url} />
      ))}
    </List.Item.Detail.Metadata>
  );
}
